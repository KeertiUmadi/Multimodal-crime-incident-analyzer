...
scores motion between frames, runs YOLOv8 on each sampled frame,
and outputs a timestamped event log.
...
Columns : Clip_ID, Timestamp, Frame_ID, Frame_File,
          Motion_Score, Event_Detected, Persons_Count, Confidence
"""
...
def process_clip(model, clip_path: str, clip_id: str, frame_dir: str) -> list:
...
def run(video_dir: str = "video/data",
    ...
    if not files:
        raise ValueError("[Video] No video files found in the directory.")
    ...
    model     = _load()
    frame_dir = os.path.join(os.path.dirname(output_csv), "frames")
    os.makedirs(frame_dir, exist_ok=True)

    all_rows = []
    for i, fname in enumerate(files, 1):
        cid = f"CLIP_{i:02d}"
        print(f"[Video] {cid} ← {fname}")
        rows = process_clip(model, os.path.join(video_dir, fname), cid, frame_dir)
    ...
    print(f"[Video] ✅ {len(df)} frames → {output_csv}\n")
    return df

""" def _demo(output_csv: str = "video/output_video.csv") -> pd.DataFrame:
    df = pd.DataFrame([
        {"Clip_ID": "CAVIAR_03", "Timestamp": "00:00:12", "Frame_ID": "FRM_036",
         "Frame_File": "CAVIAR_03_FRM_036.jpg", "Motion_Score": 0.08,
         "Event_Detected": "Person collapsing", "Persons_Count": 1, "Confidence": 0.88},
        {"Clip_ID": "CAVIAR_03", "Timestamp": "00:00:24", "Frame_ID": "FRM_072",
         "Frame_File": "CAVIAR_03_FRM_072.jpg", "Motion_Score": 0.12,
         "Event_Detected": "Crowd / Suspicious movement", "Persons_Count": 3, "Confidence": 0.91},
        {"Clip_ID": "CAVIAR_04", "Timestamp": "00:00:08", "Frame_ID": "FRM_020",
         "Frame_File": "CAVIAR_04_FRM_020.jpg", "Motion_Score": 0.05,
         "Event_Detected": "Vehicle movement", "Persons_Count": 0, "Confidence": 0.85},
    ])
    os.makedirs(os.path.dirname(output_csv), exist_ok=True)
    df.to_csv(output_csv, index=False)
